    ...
    try {
        ...
        for (const element of allElements) {
            ...
            if (team1Matches > 0 && team2Matches > 0) {
                // ✅ 过滤掉联赛标题
                // 只检查前200字符（联赛标题本身应该很短）
                const textStart = text.substring(0, 200).toUpperCase();
                const isShortElement = text.length < 200;

                const hasLeagueKeyword = (
                    textStart.includes(' LEAGUE') ||
                    textStart.includes(' SERIE') ||
                    textStart.includes(' DIVISION') ||
                    textStart.includes(' LIGA') ||
                    textStart.includes('BUNDESLIGA') ||
                    textStart.includes(' CUP') ||
                    textStart.includes('CHAMPIONSHIP') ||
                    textStart.includes('FANTASY MATCHES')
                );

                // 如果是短元素且有联赛关键词，很可能是联赛标题
                if (isShortElement && hasLeagueKeyword) {
...
async function expandAndSearchConcurrently(team1, team2) {
    console.log('[Crown Executor] 🔄 开始智能搜索（从上到下）...');

    // ✅ 第一步：先搜索已展开的内容
    console.log('[Crown Executor] 🔍 第一次搜索（默认展开的联赛）...');
    let matchResult = findMatch(team1, team2);

    if (matchResult && matchResult.score > 0) {
        console.log(`[Crown Executor] ✅ 在默认展开的联赛中找到比赛！得分${matchResult.score}`);
        return matchResult.element;
    }

    console.log('[Crown Executor] ⚠️ 在默认展开的联赛中未找到，开始逐个展开折叠的联赛...');

    // ✅ 第二步：从上到下找折叠的联赛，逐个展开并搜索
    // 使用document.body.querySelectorAll按DOM顺序获取
    ...
    for (const element of allElements) {
        ...
        if (!hasLeagueKeyword) continue;

        // ✅ 找到联赛标题，尝试点击展开
        try {
            expandedCount++;
            console.log(`[Crown Executor] 🔓 ${expandedCount} 展开: "${text.substring(0, 40)}"`);
            element.click();
            ...
            // ✅ 立即搜索
            matchResult = findMatch(team1, team2);

            if (matchResult && matchResult.score > 0) {
                console.log(`[Crown Executor] ✅ 展开"${text.substring(0, 30)}"后找到比赛！得分${matchResult.score}`);
                window.scrollTo(0, 0);
                return matchResult.element;
            }
        } catch (e) {
    ...
    console.log(`[Crown Executor] 📊 共展开${expandedCount}个联赛，仍未找到比赛`);
    ...
    return finalResult && finalResult.score > 0 ? finalResult.element : null;
}

/**
 * 展开所有折叠的联赛（保留用于Early分类）
...
async function searchMatchAcrossCategories(sportName, team1, team2, league, matchTime) {
    ...
    for (const category of searchOrder) {
        ...
        // 5. ✅ 如果是Today分类，边展开边搜索（并发优化）
        if (category === 'Today') {
            console.log('[Crown Executor] 📂 Today分类：边展开边搜索...');
            const matchElement = await expandAndSearchConcurrently(team1, team2);

            if (matchElement) {
                console.log('[Crown Executor] 🎉 在展开过程中找到比赛！');
                // 点击进入详情页
                const clickSuccess = await clickMatchToEnterDetails(matchElement, team1, team2);
                if (clickSuccess) {
                    return true;
                } else {
                    console.warn('[Crown Executor] ⚠️ 点击队名失败，继续搜索...');
                }
            } else {
                console.log('[Crown Executor] ⚠️ 展开所有联赛后仍未找到比赛');
            }
        }
        ...
        // 7. 第一次搜索比赛（对于Early分类或Today未找到的情况）
